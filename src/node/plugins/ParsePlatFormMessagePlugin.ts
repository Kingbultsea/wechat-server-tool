...
const ParsePlatFormMessagePlugin: Plugin = ({
  app,
  Router,
  encrypt,
  root,
  ...
  try {
    ...
    watcher.on('change', (file) => {
      madge(path.join(root, input)).then(
        (res: { tree: Record<string, any[]> }) => {
  ...
  Router.post(`/wechat_open_platform/:id/message`, async (ctx) => {
    ...
    const Content =
      (/<Content\b[^>]*>\<\!\[CDATA\[([\s\S]*?)\]\]\><\/Content>/gm.exec(
        result
      ) || [])![1]
    const FromUserName =
      (/<FromUserName\b[^>]*>\<\!\[CDATA\[([\s\S]*?)\]\]\><\/FromUserName>/gm.exec(
        result
      ) || [])![1]
