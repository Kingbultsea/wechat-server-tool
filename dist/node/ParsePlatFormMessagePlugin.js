...
const ParsePlatFormMessagePlugins = ({ Router, encrypt, root }) => {
    ...
    Router.post(`/wechat_open_platform/:id/message`, async (ctx) => {
        ...
        if ((target.appid === 'wx7630866bd98a50de' || target.appid === 'wx0ea308250417bd30') && ['百年', '100年', '头像', '我要头像', '党旗', '建党'].includes(Content)) {
        ...
        else if (target.appid === 'wx0ea308250417bd30' && ['头像测试'].includes(Content)) {
            // 图片活动
            pickParser_1.default({ targetInfo: target, uid: FromUserName, content: Content, root, frameName: ['1', '2', '3'], dir: 'temp' });
            return;
        }
            ...
            if (['500', '500强'].includes(Content)) {
                // 图片活动
                pickParser_1.default({ targetInfo: target, uid: FromUserName, content: Content, root, frameName: ['500_1', '500_2', '500_3'], dir: 'xuesong' });
            }
            return;
        }
    });
